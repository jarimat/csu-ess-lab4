...
Looking at these results, we see an extremely small p-value, telling us there *is* a significant relationship between forest type and channel unit (i.e., we rejected our null hypothesis).
...
Previous work has shown that forest harvesting practices can impact aquatic vertebrate biomass (Kaylor & Warren 2017). Using the `and_vertebrates` data set we can investigate this by comparing weight to forest type (clear cut or old growth). This involves a test to compare the means (average weight) among two groups (clear cut and old growth forests) using a t-test.
...
salamander_clean <- and_vertebrates %>%
  filter(species == "Coastal giant salamander") |> 
  drop_na(unittype, section, weight_g)
...
var_test_salamander <- var.test(cc_weight, og_weight)

print(var_test_salamander)
```
```{r}
t_test_salamander <- t.test(weight_g ~ section, data = salamander_clean, var.equal = FALSE)

print(t_test_salamander)
```
There seems to be a significant relationship between the weight of the salamander and whether they are in a clear cut or old growth forest.
...
trout_clean$length_category <- cut(trout_clean$length_1_mm, breaks = 5)

trout_clean$weight_category <- cut(trout_clean$weight_g, breaks = 5)

troutlength_table <- table(trout_clean$length_category, trout_clean$weight_category)

chisq.test(troutlength_table)
```
...
This returns a p-value of 0.71, so we accept the null hypothesis that this data fits a normal distribution assumption.
...
Similar to the `var.test()` function you've used before, the *null hypothesis* of the Levene's test is that the variances *are equal* across all groups. Given this small p-value (denoted by the 'Pr(\>F)' value) we confirm that the variances of our groups are _not_ equal. Therefore we need to use a Welch ANOVA (`oneway.test`), which we specify by setting `var.equal = FALSE`:
...
Our data suggests that there is a positive correlation between latitude and carapace width, with higher latitudes generally experiencing wider mean carapace width and lower latitudes generally experiencing shorter mean carapace width.
...
```{r}
watertemp_lm <- lm(size ~ water_temp_sd, data = pie_crab)

summary(watertemp_lm)
...
The resulting p-value from the linear regression suggests that there is an insignificant relationship between annual water temperature variation and carapace width.
...
```{r}
carapace_mlm <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)

summary(carapace_mlm)
```
The multiple linear regression points towards there not being a significant relationship between annual water temperature variation and carapace width as well as annual air temperature variation and carapace width. It does suggest that there is a significant relationship between latitude and carapace width, however.
